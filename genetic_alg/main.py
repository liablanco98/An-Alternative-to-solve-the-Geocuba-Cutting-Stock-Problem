...
def ret_output(stock: Stock, skip: bool, function_type: int, iterations_count: int, ellitism: int , higher_matters: int, population_size: int):
    '''
        Parameters:
    ...
    '''
    genetic = GA(stock, skip, function_type, iterations_count,
                 ellitism, higher_matters, population_size)
    ...
    for index in range(len(best_subjects)):
        ...
        # second = sorted(subject.pattern, key=lambda x: x[0])
...
def main(pieces_list: list[tuple[tuple[int, int, int], int]], sheet: tuple[int, int] = (700, 500), space: int = 0, incompatible: list[tuple[int, int]] = [], skip: bool = True, funct_type: int = 1, iterations_count: int = 1000, ellitism: int = 20, higher_matters: int = 40, population_size: int = -1):
    '''
    ...
    return ret_output(stock=stock, skip=skip, function_type=funct_type, iterations_count=iterations_count, ellitism=ellitism, higher_matters=higher_matters, population_size=population_size)
